...
export function ExcavatorDetailDialog({ excavator, onClose }: ExcavatorDetailDialogProps) {
  ...
  const truckSummaries = useMemo(() => {
    ...
    excavator.records.forEach(row => {
      ...
      if (row.Material) summary.materiais.add(row.Material);
    });
  ...
  const exportToPDF = () => {
    ...
    doc.setFontSize(14);
    doc.text('Detalhamento por Caminhão', 14, finalY + 15);

    const truckData = truckSummaries.map(truck => [
      truck.prefixo,
      truck.motorista,
      truck.viagens.toString(),
      `${truck.volume.toFixed(0)} m³`,
      Array.from(truck.locais).join(', '),
      Array.from(truck.materiais).join(', '),
    ]);

    autoTable(doc, {
      head: [['Caminhão', 'Motorista', 'Viagens', 'Volume', 'Locais', 'Materiais']],
      body: truckData,
      startY: finalY + 20,
      theme: 'striped',
      styles: { fontSize: 8 },
          ...
          <Table>
            ...
            <TableBody>
              {truckSummaries.map((truck) => (
                <TableRow key={truck.prefixo} className="data-table-row">
                  <TableCell>
                    <div>
                  ...
                  <TableCell>
                    <div className="flex flex-wrap gap-1">
                      {Array.from(truck.materiais).map(mat => (
